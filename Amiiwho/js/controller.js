...
window.addEventListener("load", () => {
  document.getElementsByClassName("homepage-text").display = "visble";
});
...
function getPicture(amiibo) {
  document.getElementById(
    "picture"
  ).innerHTML = `<img src="${amiibo.image}" class="picture"></img>`;
}

function getInformation(amiibo) {
  document.getElementById(
    "information"
  ).innerHTML = `<br><span class="text">Amiibo : </span>${amiibo.name}</br>
...
function giveInteractionBooks() {
  for (let i = 0; i < bookmarks.length; i++) {
    document
      .getElementById(`bookmark-${bookmarks[i][0]}`)
...
function makeDeletion() {
  for (let i = 0; i < bookmarks.length; i++) {
    document
      .getElementById(`delete-${bookmarks[i][0]}`)
      .addEventListener("click", (e) => {
        e.preventDefault();
        let currentBookmark = e.target.parentElement.id.slice(7);
        bookmarks = bookmarks.filter(
          (bookmark) => bookmark[0] !== currentBookmark
        );
